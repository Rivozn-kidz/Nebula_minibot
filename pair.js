...
⏱️ Uptime: ${formatUptime(process.uptime())}`;
            ...
            case 'gitclone': {
                try {
                    ...
                    await bot.sendMessage(from, { document: { url: zipUrl }, fileName: filename, mimetype: 'application/zip' }, { quoted: kabi });
            ...
            case 'save': {
                try {
                    ...
                    const mediaBuffer = await bot.downloadMediaMessage(quoted);
                    ...
                    await bot.sendMessage(m.sender, payload, { quoted: kabi });
...
case 'owner': {
    try {
        ...
        reply("❌ Unable to fetch owner info.");
    }
    break;
}
...
case 'pinggc': {
    ...
    const msg = await reply("📡 Pinging group...");
    const end = Date.now();
    await reply(`✅ Group response time: ${end - start}ms`);
    break;
}
              case 'dllogo': {
                const q = args.join(" ");
